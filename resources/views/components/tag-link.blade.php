<div class="inline-flex items-stretch bg-gray-200 dark:bg-gray-900 rounded-full text-sm font-semibold text-gray-700 dark:text-gray-300">
    ...
    @if(auth()->check() && isset($work))
        <form class="-ml-1" action="{{ route('works.tags.remove', [$work, $tag]) }}" method="post">
            @csrf
            @method('DELETE')
            <button type="submit" class="text-gray-700 hover:text-gray-900 dark:text-gray-300 dark:hover:text-gray-100 h-full px-2 -ml-1 z-10" title="{{ __('Remove tag') }}">
